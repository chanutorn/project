...
$sql = "SELECT * FROM tbl_product as p INNER JOIN tbl_type as t ON p.type_id = t.type_id WHERE p.type_id = $type_id ORDER BY `p`.`p_qty` DESC";
$result = mysqli_query($con, $sql);

// Query สินค้าที่มี p_view มากที่สุด
$sql_v = "SELECT * FROM tbl_product AS p INNER JOIN tbl_type AS t ON p.type_id = t.type_id WHERE p.p_qty > 0 AND p.p_view > 40 AND p.type_id = $type_id ORDER BY p.p_view DESC LIMIT 1";
